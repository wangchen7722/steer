...
on_check: |
  ...
  <check>{{ check }}</check>
---
This is the openspec-generate-specs workflow. Current position: **coverage-guard** -- the gate that `openspec validate` CANNOT provide. Validate checks structure only; it happily passes a spec that silently dropped half the project's existing behavior requirements. This gate runs ONLY for target repos where prior `openspec/specs/` existed (the workflow already branched on env.md before calling this step).
...
2. Diff requirement-by-requirement: each existing `### Requirement:` (and its scenarios) must be present in the regenerated spec in that repo, preserving the behavior contract and any true contract facts (IDs, values, timeouts, signatures, ABI/protocol/policy/layout/lifecycle details).
3. Any existing requirement/scenario/behavior contract absent from the generated spec is a REGRESSION -- add it to that repo's spec, then re-run `cd <repo> && openspec validate <cap> --type spec` until exit 0.

If an existing requirement is implementation-level or overly literal, preserve it during this refresh for regression safety; do NOT silently rewrite or drop it. New content added by this workflow should follow the behavior-contract standard.
...
- **Per-capability requirement diff**:
  - `<!-- cap -->`: <!-- M existing behavior requirements reproduced; K net-new behavior requirements added; OR "REGRESSION: <requirement> missing -> fixed and re-validated via `cd <repo> && openspec validate`" -->

### Aggregate

- **Repos with prior specs**: <!-- list -->
- **Total existing capabilities reproduced**: <!-- N -->
- **Total existing behavior requirements reproduced**: <!-- M -->
- **Total net-new behavior requirements added**: <!-- K -->
- **Regressions found and fixed**: <!-- count, or "none" -->
- **Verdict**: <!-- PASS (every existing capability + requirement/scenario/behavior contract reproduced across all repos, plus net-new additions) or FAIL (list unresolved regressions per repo) -->
</template>
...
- Any existing requirement/scenario/behavior contract absent from the generated spec is a regression. Fix it by ADDING to that repo's spec (preserve existing content), then re-run `cd <repo> && openspec validate <cap> --type spec` until exit 0.
- Preserve existing implementation-level wording if it already exists; semantic cleanup is a separate reviewed change. Do not use old implementation narration as the model for new generated requirements.
