...
- **Cross-repo seam** (cross-repo units only): <!-- bidirectional: which files/symbols in repo A call or depend on which in repo B (A→B), and which in B are called by or depended on by A (B←A); also the cross-repo signal (Depends-On MR, close merge timestamps, doc/code cross-references); omit for single-repo units -->
